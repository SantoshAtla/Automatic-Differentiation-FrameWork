...
def d_truediv(operands, v):
    ...
    return \
        ((operands[0].derivative(v) * operands[1].value -\
        operands[0].value * operands[1].derivative(v)) / (operands[1] * operands[1])).value
...
def d_pow(operands, v):
    ...
    if operands[0].value > 0:
        return \
           (((operands[1]*(operands[0]**(operands[1]-1)))*operands[0].derivative(v)) +\
            ((operands[0]**operands[1])*math.log(operands[0].value)*(operands[1].derivative(v)))).value
    else:
        return \
           ((operands[1]*(operands[0]**(operands[1]-1)))*operands[0].derivative(v)).value
